...
#### Big sprites
Sprites defined through the formatting `:generaton/pokemon:` are converted to `<img>` tags with a `src` pointing to their files on Pokemon Showdown!'s sprite directory and an `alt` attribute correctly (in most cases) naming the Pokemon. If `generation` features the word "ani", then the extension will be `gif`, otherwise, it will be `png`. In addition, "rb" points to the `gen1` directory, "gs" to the `gen2` directory, "rs" to the `gen3` directory, "dp" to the `gen4` directory, "bw" to the `gen5ani` directory, and anything else to the `ani` directory.
