...
export function addBigSprites(article: string): string {
    const sprites: RegExpMatchArray | null = article.match(/:[a-z][a-z]\/.*?:/gi);

    let gen: ObjectKey;
    let pokemon: string;
    let capitalizedPokemon: string;
    let directory: string;
    let extension: string;

    if (sprites !== null) {
        for (const sprite of sprites) {
            gen = sprite[1] + sprite[2] as ObjectKey;
            ...
            const edgeCase = edgeCaseNames[pokemon]; // object

            if (edgeCase !== undefined) {
                article = article
                    .replace(sprite,
                    `<img src="//play.pokemonshowdown.com/sprites/${directory}/${edgeCase.name}.${extension}" alt="${edgeCase.alt}" />`);
            } else {
                const regionalForme = handleRegionalForme(pokemon) // object;
